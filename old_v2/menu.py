...
class LabyrinthResolution(SubMenu):
    def __init__(self, parent, labyrinth):
        ...
        self.elements.append(Button("Retry", action=self.parent.retry, pos=(SCREEN_WIDTH - 200, 70)))
...
class MainMenu(SubMenu):
    def __init__(self, parent):
        ...
        self.elements.append(Button(text="Play", action=self.parent.play, pos=(10, 10), size=(100, 50)))
        self.elements.append(
            Button(
        ...
        self.elements.append(
            Button(
                "Quitter",
                action=lambda: setattr(self.parent, "running", False),  # Modify attribute with a lambda function
                pos=(10, 130),
                size=(100, 50),
            )
        )
...
class Menu(object):
    def __init__(self):
        ...
        self.frames = []
        self.frameCount = 0
        self.FPS = -1  # Une valeur négative signifie que le FPS n'a pas pu être calculé.
        self.G = Graphics()
                ...
                if event.type == pygame.QUIT:
                    ...
                    for button in self.screenStack[-1].elements:
                        if button.TYPE == "Button":
                            if (
                                button.pos[0] < event.pos[0] < button.pos[0] + button.size[0]
                                and button.pos[1] < event.pos[1] < button.pos[1] + button.size[1]
                            ):
                                if button.action:
                                    button.action()
